...
// --- RESET POR FASES (LÓGICA SOLICITADA) ---
app.post('/reset-tournament/:id', (req, res) => {
    ...
    if (target === 'all') {
        ...
        db.query('DELETE FROM matches WHERE tournament_id = ? AND phase LIKE ?', [tId, `%${phaseName}%`], () => {
            res.send(`Reset ${target} OK`);
        });
    }
});

// --- EL RESTO DE RUTAS (Teams, Players, Goals, Stats) SE MANTIENEN IGUAL QUE v3.4 ---
...
app.listen(process.env.PORT || 3001, '0.0.0.0', () => console.log("🚀 v3.5 backend ready"));