...
// Railway detectará estas variables automáticamente si las vinculas
...
// Verificación de conexión en la nube
db.getConnection((err, connection) => {
    if (err) {
        console.error("❌ ERROR CONECTANDO A MYSQL EN LA NUBE:", err.message);
    } else {
...
app.post('/teams', (req, res) => {
    ...
    [tournament_id, name, logo_url, group_num], (err, result) => {
        if (err) return res.status(500).send(err);
        res.send({ id: result.insertId });
    });
});
...
app.put('/matches/:matchId', (req, res) => {
    ...
    db.query(sql, [team_a_id, team_b_id, date, field, referee, team_a_goals, team_b_goals, played, req.params.matchId], (err) => {
        if (err) return res.status(500).send(err);
        res.send("OK");
    });
});
...
app.post('/generate-schedule/:id', (req, res) => {
    const tId = req.params.id; const { startTime } = req.body;
    db.query('SELECT * FROM teams WHERE tournament_id = ?', [tId], (err, teams) => {
        if (teams.length < 8) return res.status(400).send("Faltan equipos");
...
app.post('/generate-league/:id', (req, res) => {
    ...
    db.query('SELECT * FROM teams WHERE tournament_id = ?', [tId], (err, teams) => {
        ...
        db.query('INSERT INTO matches (tournament_id, team_a_id, team_b_id, match_date, field, phase) VALUES ?', [m], (err2) => res.send("OK"));
    });
});

app.post('/generate-playoffs-custom/:id', (req, res) => {
    const { phase, pairings } = req.body;
    const matchesArr = pairings.map(p => [req.params.id, p.a, p.b, new Date().toISOString().slice(0, 19).replace('T', ' '), p.field, phase]);
