...
app.get('/matches/:tournamentId', (req, res) => {
    ...
    db.query(sql, [req.params.tournamentId], (err, result) => { if (err) return res.status(500).send(err); res.send(result); });
});
...
app.put('/matches/:matchId', (req, res) => {
    ...
    [team_a_id, team_b_id, date, field, referee, team_a_goals, team_b_goals, played, req.params.matchId], (err) => {
        if (err) return res.status(500).send(err); res.send("OK");
    });
});
...
app.post('/generate-schedule/:id', (req, res) => {
    ...
    db.query('SELECT * FROM teams WHERE tournament_id = ?', [tId], (err, teams) => {
        ...
        db.query("INSERT INTO matches (tournament_id, team_a_id, team_b_id, match_date, field, phase) VALUES ?", [m], (errIns) => {
            if (errIns) return res.status(500).send("Error insertando: " + errIns.message);
            res.send("OK");
        });
    });
});
...
app.post('/generate-league/:id', (req, res) => {
    ...
    db.query('SELECT * FROM teams WHERE tournament_id = ?', [tId], (err, teams) => {
        ...
        db.query("INSERT INTO matches (tournament_id, team_a_id, team_b_id, match_date, field, phase) VALUES ?", [m], (errIns) => {
            if (errIns) return res.status(500).send("Error insertando liga: " + errIns.message);
            res.send("OK");
        });
    });
});
...
app.post('/generate-playoffs-custom/:id', (req, res) => {
    ...
    const tId = parseInt(req.params.id);
    const matchesArr = pairings.map(pair => [tId, pair.a, pair.b, formatDateToMySQL(new Date()), pair.field, phase]);
    db.query('INSERT INTO matches (tournament_id, team_a_id, team_b_id, match_date, field, phase) VALUES ?', [matchesArr], (err) => {
        if (err) return res.status(500).send(err.message); res.send("OK");
    });
});
