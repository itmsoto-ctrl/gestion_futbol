...
// --- CONFIGURACIÓN DE CONEXIÓN FLEXIBLE PARA RAILWAY ---
...
// Chivato de conexión en los logs
...
// --- 7. GENERADORES DE ENFRENTAMIENTOS ---

// Campeonato: 8 equipos, 2 grupos, 12 partidos iniciales
app.post('/generate-schedule/:id', (req, res) => {
    ...
    db.query('SELECT * FROM teams WHERE tournament_id = ?', [tId], (err, teams) => {
        ...
        if (g1.length !== 4 || g2.length !== 4) {
            return res.status(400).send(`Faltan equipos en grupos (G1:${g1.length}, G2:${g2.length}). Deben ser 4 y 4.`);
        }
...
// Activación de fases de Playoff (Cuartos, Semis, Final)
app.post('/generate-playoffs-custom/:id', (req, res) => {
    const { phase, pairings } = req.body;
    const tId = parseInt(req.params.id);
    const matchesArr = pairings.map(pair => [tId, pair.a, pair.b, formatDateToMySQL(new Date().toISOString()), pair.field, phase]);
...
// --- 8. ESTADÍSTICAS (PICHICHI Y ZAMORA) ---
