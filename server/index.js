...
app.post('/teams', (req, res) => {
    ...
    db.query('INSERT INTO teams (tournament_id, name, logo_url, group_num) VALUES (?, ?, ?, ?)', [tournament_id, name, logo_url, group_num], (err, result) => {
        if (err) return res.status(500).send(err);
        res.send(result);
    });
});
...
app.get('/players/:tournamentId', (req, res) => {
    db.query('SELECT p.*, t.name as team_name FROM players p JOIN teams t ON p.team_id = t.id WHERE t.tournament_id = ?', [req.params.tournamentId], (err, result) => {
        if (err) return res.status(500).send(err);
        res.send(result);
    });
});
...
app.put('/matches/:matchId', (req, res) => {
    ...
    [team_a_id, team_b_id, date, field, referee, team_a_goals, team_b_goals, played, req.params.matchId], (err) => {
        if (err) return res.status(500).send(err);
        res.send("OK");
    });
});
...
app.post('/add-player-goal', (req, res) => {
    ...
    db.query('INSERT INTO goals (match_id, player_id, team_id) VALUES (?, ?, ?)', [match_id, player_id, team_id], (err) => {
        if (err) return res.status(500).send(err);
        db.query(`UPDATE matches SET ${team_side} = ${team_side} + 1, played = 1 WHERE id = ?`, [match_id], (err2) => {
...
app.post('/generate-schedule/:id', (req, res) => {
    ...
    db.query('SELECT * FROM teams WHERE tournament_id = ?', [tId], (err, teams) => {
        if (err) return res.status(500).send(err);
        if (teams.length < 8) return res.status(400).send("Faltan equipos (se necesitan 8)");
        ...
        for (let i = 0; i < 3; i++) {
            const time = new Date(start.getTime() + (i * 30 * 60000)).toISOString().slice(0, 19).replace('T', ' ');
            if (g1[p[i*2]]) m.push([tId, g1[p[i*2][0]].id, g1[p[i*2][1]].id, time, 1, 'grupo']);
            if (g1[p[i*2+1]]) m.push([tId, g1[p[i*2+1][0]].id, g1[p[i*2+1][1]].id, time, 2, 'grupo']);
        }
        for (let i = 0; i < 3; i++) {
            const time = new Date(start.getTime() + ((i + 3) * 30 * 60000)).toISOString().slice(0, 19).replace('T', ' ');
            if (g2[p[i*2]]) m.push([tId, g2[p[i*2][0]].id, g2[p[i*2][1]].id, time, 1, 'grupo']);
            if (g2[p[i*2+1]]) m.push([tId, g2[p[i*2+1][0]].id, g2[p[i*2+1][1]].id, time, 2, 'grupo']);
        }
        db.query('INSERT INTO matches (tournament_id, team_a_id, team_b_id, match_date, field, phase) VALUES ?', [m], (errIns) => {
            if (errIns) return res.status(500).send(errIns);
            res.send("OK");
        });
    });
});
...
app.post('/generate-league/:id', (req, res) => {
    ...
    db.query('SELECT * FROM teams WHERE tournament_id = ?', [tId], (err, teams) => {
        if (err) return res.status(500).send(err);
        if (teams.length < 2) return res.status(400).send("No hay suficientes equipos");
        let m = [];
...
app.post('/generate-playoffs-custom/:id', (req, res) => {
    ...
    const tId = parseInt(req.params.id);
    const matchesArr = pairings.map(p => [tId, p.a, p.b, new Date().toISOString().slice(0, 19).replace('T', ' '), p.field, phase]);
