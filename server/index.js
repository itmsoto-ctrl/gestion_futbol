...
// Chivato de conexión en logs de Railway
...
// Helper para limpiar la fecha de la "T" de HTML y milisegundos para MySQL
...
app.put('/matches/:matchId', (req, res) => {
    ...
    db.query(sql, [team_a_id, team_b_id, formatDateToMySQL(match_date), field, referee, team_a_goals || 0, team_b_goals || 0, played || 0, req.params.matchId], (err) => {
        if (err) return res.status(500).send(err);
        res.send("OK");
    });
});

// --- 6. GOLES (SUMAR, RESTAR Y LISTAR) ---
app.get('/goals/:tournamentId', (req, res) => {
    const sql = `SELECT g.* FROM goals g JOIN matches m ON g.match_id = m.id WHERE m.tournament_id = ?`;
    db.query(sql, [req.params.tournamentId], (err, result) => {
        if (err) return res.status(500).send(err);
        res.send(result);
    });
...
// --- 7. GENERADORES (LÓGICA DE TIEMPO +30 MIN) ---
...
app.post('/generate-schedule/:id', (req, res) => {
    ...
    db.query('SELECT * FROM teams WHERE tournament_id = ?', [tId], (err, teams) => {
        ...
        for (let i = 0; i < 3; i++) {
            const time = formatDateToMySQL(new Date(start.getTime() + (i * 30 * 60000)).toISOString());
            m.push([tId, g1[p[i*2][0]].id, g1[p[i*2][1]].id, time, 1, 'grupo']);
            m.push([tId, g1[p[i*2+1][0]].id, g1[p[i*2+1][1]].id, time, 2, 'grupo']);
        }
        for (let i = 0; i < 3; i++) {
            const time = formatDateToMySQL(new Date(start.getTime() + ((i + 3) * 30 * 60000)).toISOString());
...
app.post('/activate-phase/:id', (req, res) => {
    ...
    // Busca la fecha del último partido para sumar 30 minutos
    db.query('SELECT MAX(match_date) as lastDate FROM matches WHERE tournament_id = ?', [tId], (err, result) => {
        let nextTime = (result[0].lastDate) ? new Date(new Date(result[0].lastDate).getTime() + 30 * 60000) : new Date();
        const timeStr = formatDateToMySQL(nextTime.toISOString());
        
        const matchesArr = pairings.map(pair => [tId, pair.a, pair.b, timeStr, pair.field, phase]);
