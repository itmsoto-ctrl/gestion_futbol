...
// Helper para tratar fechas como texto y evitar desfases horarias
...
app.get('/tournaments', (req, res) => {
    db.query('SELECT * FROM tournaments', (err, result) => res.send(result));
});
...
app.put('/matches/:matchId', (req, res) => {
    ...
    // Guardamos la fecha tal cual viene del cliente (texto)
...
// --- ACTIVACIÓN DE FASES CON TIEMPOS ESCALONADOS ---
app.post('/activate-phase/:id', (req, res) => {
    ...
    db.query('SELECT MAX(match_date) as lastDate FROM matches WHERE tournament_id = ?', [tId], (err, result) => {
        ...
        baseTime.setMinutes(baseTime.getMinutes() + 30); // Primer bloque 30 min después

        const matchesArr = pairings.map((pair, index) => {
            let matchTime = new Date(baseTime);
            if (phase === 'cuartos') {
                if (index === 2) matchTime.setMinutes(matchTime.getMinutes() + 30); // 18:00
                if (index === 3) matchTime.setMinutes(matchTime.getMinutes() + 60); // 18:30
            }
            return [tId, pair.a, pair.b, formatDateStr(matchTime), pair.field, phase];
        });
...
// Mismas rutas que antes para goals, teams, players...
...
app.listen(process.env.PORT || 3001, '0.0.0.0', () => console.log("🚀 Servidor v3.2"));