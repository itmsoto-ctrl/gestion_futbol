...
app.post('/remove-player-goal', (req, res) => {
    ...
    db.query('DELETE FROM goals WHERE match_id = ? AND player_id = ? AND team_id = ? ORDER BY id DESC LIMIT 1', [match_id, player_id, team_id], (err, r) => {
        ...
        } else res.status(404).send("No goals");
    });
});

// --- RESET MAESTRO (LÓGICA ASEGURADA) ---
app.post('/reset-tournament/:id', (req, res) => {
    const tId = req.params.id;
    // 1. Borrar todos los goles asociados a partidos de este torneo
    db.query('DELETE FROM goals WHERE match_id IN (SELECT id FROM matches WHERE tournament_id = ?)', [tId], (err) => {
        // 2. Borrar partidos que NO son fase grupo
        db.query('DELETE FROM matches WHERE tournament_id = ? AND phase != "grupo"', [tId], (err2) => {
            // 3. Resetear a cero los partidos de fase grupo (pero NO borrarlos)
            db.query('UPDATE matches SET played = 0, team_a_goals = 0, team_b_goals = 0 WHERE tournament_id = ? AND phase = "grupo"', [tId], (err3) => {
                res.send("Torneo Reseteado");
            });
        });
    });
});
...
app.post('/activate-phase/:id', (req, res) => {
    ...
    db.query('SELECT MAX(match_date) as last FROM matches WHERE tournament_id = ?', [tId], (err, r) => {
        ...
        const matchesArr = req.body.pairings.map((p, i) => {
            ...
            // Lógica de 17:30 (C1,C2), 18:00 (C1), 18:30 (C2) para Cuartos
            if (req.body.phase === 'cuartos') { 
                if(i === 2) mt.setMinutes(mt.getMinutes() + 30); 
                if(i === 3) mt.setMinutes(mt.getMinutes() + 60); 
            }
...
app.listen(process.env.PORT || 3001, '0.0.0.0', () => console.log("🚀 v3.4 en marcha"));