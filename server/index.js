...
// Helper para guardar la fecha tal cual se escribe en el móvil
...
app.put('/matches/:matchId', (req, res) => {
    ...
    db.query(sql, [team_a_id, team_b_id, formatDateToMySQL(match_date), field, referee, team_a_goals, team_b_goals, played, req.params.matchId], (err) => res.send("OK"));
});
...
app.post('/generate-schedule/:id', (req, res) => {
    ...
    db.query('SELECT * FROM teams WHERE tournament_id = ?', [tId], (err, teams) => {
        ...
        if (g1.length !== 4 || g2.length !== 4) return res.status(400).send("Grupos incompletos");
