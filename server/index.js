...
app.post('/activate-phase/:id', (req, res) => {
    ...
    db.query('SELECT MAX(match_date) as last FROM matches WHERE tournament_id = ?', [tId], (err, r) => {
        ...
        const mArr = pairings.map((p, i) => {
            let mt = new Date(start);
            if (phase === 'cuartos') {
                if(i === 2 || i === 3) mt.setMinutes(mt.getMinutes() + 30); 
            }
...
app.get('/matches/:tId', (req, res) => {
    db.query(`SELECT m.*, t1.name as team_a_name, t1.logo_url as team_a_logo, t2.name as team_b_name, t2.logo_url as team_b_logo 
              FROM matches m LEFT JOIN teams t1 ON m.team_a_id = t1.id LEFT JOIN teams t2 ON m.team_b_id = t2.id 
              WHERE m.tournament_id = ? ORDER BY m.match_date ASC, m.id ASC`, [req.params.tId], (err, r) => res.send(r));
});
app.put('/matches/:id', (req, res) => {
    ...
    db.query('UPDATE matches SET team_a_goals=?, team_b_goals=?, played=?, referee=?, match_date=? WHERE id=?', [team_a_goals, team_b_goals, played, referee, date, req.params.id], () => res.send("OK"));
});
...
app.listen(process.env.PORT || 3001, '0.0.0.0', () => console.log("🚀 v3.9 listo"));