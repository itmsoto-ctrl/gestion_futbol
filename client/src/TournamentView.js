...
const TournamentView = ({ user }) => {
    ...
    const getWinner = (m) => {
        if (!m || !m.played || m.team_a_goals === m.team_b_goals) return { name: '???', id: null };
    ...
    const handleGoal = async (e, mId, pId, tId, side, action) => {
        e.stopPropagation(); // BLOQUEO CLAVE: Detiene que el click cierre la caja
    ...
    const handleActivate = async (phase) => {
        ...
        let pairings = [];
        const qM = matches.filter(m => m.phase.includes('cuarto'));
        const sM = matches.filter(m => m.phase.includes('semi'));
        ...
        else if (phase === 'semifinal') pairings = [{a: getWinner(qM[0]).id, b: getWinner(qM[3]).id, field: 1}, {a: getWinner(qM[1]).id, b: getWinner(qM[2]).id, field: 2}];
        else if (phase === 'final') pairings = [{a: getWinner(sM[0]).id, b: getWinner(sM[1]).id, field: 1}];
                ...
                <div style={{ display: 'flex', padding: '20px 5px 10px 5px', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExp && isAdmin && !isFin && players.filter(p => p.team_id === m.team_a_id).map(p => (
                            <div key={p.id} style={{display:'flex', alignItems:'center', marginBottom:10}}>
                                <button onClick={(e) => handleGoal(e, m.id, p.id, m.team_a_id, 'team_a_goals', 'add')} style={{padding:'14px 4px', flex:1, fontSize:13, fontWeight:'bold', borderRadius:10, background: '#f0f4ff', border:'1px solid #ccd'}}>⚽ {p.name} {allGoals.filter(g => g.player_id === p.id && g.match_id === m.id).length > 0 ? `(${allGoals.filter(g => g.player_id === p.id && g.match_id === m.id).length})` : ''}</button>
                                <button onClick={(e) => handleGoal(e, m.id, p.id, m.team_a_id, 'team_a_goals', 'remove')} style={{marginLeft:5, color:'red', background:'none', border:'none', fontSize:25}}>×</button>
                            </div>
                        ))}
                    </div>
                    ...
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExp && isAdmin && !isFin && players.filter(p => p.team_id === m.team_b_id).map(p => (
                            <div key={p.id} style={{display:'flex', alignItems:'center', marginBottom:10}}>
                                <button onClick={(e) => handleGoal(e, m.id, p.id, m.team_b_id, 'team_b_goals', 'remove')} style={{marginRight:5, color:'red', border:'none', background:'none', fontSize:25}}>×</button>
                                <button onClick={(e) => handleGoal(e, m.id, p.id, m.team_b_id, 'team_b_goals', 'add')} style={{padding:'14px 4px', flex:1, fontSize:13, fontWeight:'bold', borderRadius:10, background: '#fff0f0', border:'1px solid #dcc'}}>{p.name} {allGoals.filter(g => g.player_id === p.id && g.match_id === m.id).length > 0 ? `(${allGoals.filter(g => g.player_id === p.id && g.match_id === m.id).length})` : ''} ⚽</button>
                            </div>
                        ))}
                    </div>
                </div>
    ...
    if (loading) return <div style={{ padding: '100px 0', textAlign: 'center' }}>Cargando v3.10...</div>;
        ...
        <div style={{ padding: '0 0 50px 0', fontFamily: 'Arial', maxWidth: '600px', margin: 'auto', background: '#f8f9fa', minHeight: '100vh' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#000', padding: '15px 20px', color: 'white', position: 'sticky', top: 0, zIndex: 1000 }}>
                ...
                <div style={{display:'flex', gap:5}}>
                    {isAdmin && <button onClick={() => setShowActivateMenu(!showActivateMenu)} style={{background:'#28a745', color:'#fff', border:'none', padding:'8px 10px', borderRadius:8, fontSize:11, fontWeight:'bold'}}>ACTIVAR ⚡</button>}
                    {isAdmin && <button onClick={() => setShowResetMenu(!showResetMenu)} style={{background:'#dc3545', color:'#fff', border:'none', padding:'8px 10px', borderRadius:8, fontSize:11, fontWeight:'bold'}}>RESET ⚙️</button>}
                </div>
            ...
            {showResetMenu && (
                <div style={{ background: '#fff', padding: 15, borderBottom: '2px solid #dc3545', display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
                    <button onClick={() => handleReset('all')} style={{ padding: 10, background: '#dc3545', color: '#fff', borderRadius: 5 }}>TODO</button>
                    <button onClick={() => handleReset('cuartos')} style={{ padding: 10, background: '#333', color: '#fff', borderRadius: 5 }}>CUARTOS</button>
                    <button onClick={() => handleReset('semifinal')} style={{ padding: 10, background: '#333', color: '#fff', borderRadius: 5 }}>SEMIS</button>
                    <button onClick={() => handleReset('final')} style={{ padding: 10, background: '#333', color: '#fff', borderRadius: 5 }}>FINAL</button>
                </div>
            )}
            ...
            {showActivateMenu && (
                <div style={{ background: '#fff', padding: 15, borderBottom: '2px solid #28a745', display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
                    <button onClick={() => handleActivate('cuartos')} style={{ padding: 10, background: '#28a745', color: '#fff', borderRadius: 5 }}>CUARTOS</button>
                    <button onClick={() => handleActivate('semifinal')} style={{ padding: 10, background: '#28a745', color: '#fff', borderRadius: 5 }}>SEMIS</button>
                    <button onClick={() => handleActivate('final')} style={{ padding: 10, background: '#28a745', color: '#fff', borderRadius: 5 }}>FINAL</button>
                </div>
            )}
            ...
            <div style={{ padding: '15px' }}>
                            ...
                            <div style={{ flex: 1.2 }}>
                                {[ [1,8], [2,7], [3,6], [4,5] ].map((pair, i) => (
                                    <div key={i} style={{ background: '#f8f9fa', marginBottom: '5px', padding: '5px', borderRadius: '5px', border: '1px solid #eee', fontSize: '9px' }}>
                                        ...
                                        <div style={{display:'flex', justifyContent:'space-between', color: qMatches[i] ? '#000' : '#007bff', borderTop:'1px solid #fff'}}><span>{qMatches[i] ? qMatches[i].team_b_name : getTeamByRank(pair[1]).name}</span><b>{qMatches[i]?.team_b_goals ?? ''}</b></div>
                                    </div>
                                ))}
                            </div>
                ...
                <div style={{ marginTop: '50px', background: 'white', padding: '20px', borderRadius: '20px', boxShadow: '0 4px 15px rgba(0,0,0,0.1)' }}>
                    <h4 style={{margin: '0 0 15px 0', borderLeft: '5px solid #ffc107', paddingLeft: 10}}>🏆 Goleadores (Pichichi)</h4>
