...
const TournamentView = ({ user }) => {
    ...
    const handleGoal = async (e, mId, pId, tId, side, action) => {
        e.stopPropagation(); // ESCUDO: No cierra la caja
            ...
            <div onClick={() => isAdmin && setExpandedMatchId(isExp ? null : m.id)}
                ...
                <div style={{ display: 'flex', padding: '20px 5px 10px 5px', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ flex: '0 0 28%', textAlign: 'center', paddingTop: '10px' }}>
                        ...
                        {isExp && isAdmin && (
                            <button onClick={(e) => { e.stopPropagation(); isFin ? axios.put(`${API_URL}/matches/${m.id}`, {...m, played:0}).then(()=>loadData()) : axios.put(`${API_URL}/matches/${m.id}`, {...m, played:1}).then(()=>loadData()); }} style={{marginTop:20, padding:'10px 15px', background: isFin ? '#ffc107' : '#28a745', color: isFin ? '#000' : '#fff', border:'none', borderRadius:10, fontSize:12, fontWeight:'bold', width:'100%'}}>
    ...
    if (loading) return <div style={{ padding: '100px 0', textAlign: 'center' }}>v3.8.0 - Cargando...</div>;
        ...
        <div style={{ padding: '0 0 50px 0', fontFamily: 'Arial', maxWidth: '600px', margin: 'auto', background: '#f8f9fa', minHeight: '100vh' }}>
            ...
            {showActivateMenu && (
                <div style={{ background: '#fff', padding: 15, borderBottom: '2px solid #28a745', display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
                    <button onClick={() => handleActivate('cuartos')} style={{ padding: 10, background: '#28a745', color: '#fff', borderRadius: 5, fontSize:10, border:'none' }}>CUARTOS</button>
                    <button onClick={() => handleActivate('semifinal')} style={{ padding: 10, background: '#28a745', color: '#fff', borderRadius: 5, fontSize:10, border:'none' }}>SEMIS</button>
                    <button onClick={() => handleActivate('final')} style={{ padding: 10, background: '#28a745', color: '#fff', borderRadius: 5, fontSize:10, border:'none' }}>FINAL</button>
                </div>
            )}
