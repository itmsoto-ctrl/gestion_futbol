...
const TournamentView = ({ user }) => {
    ...
    const standings = useMemo(() => {
        let table = {};
        teams.forEach(t => { table[t.id] = { id: t.id, name: t.name, pts: 0, gf: 0, gc: 0 }; });
        matches.forEach(m => {
            if (m.played && m.phase.includes('grupo')) {
    ...
    const MatchCard = ({ m }) => {
        ...
        return (
            <div style={{ border: '1px solid #ddd', borderRadius: '15px', background: isFinished ? '#e8f5e9' : 'white', marginBottom: '15px', overflow:'hidden', boxShadow:'0 3px 6px rgba(0,0,0,0.05)' }}>
                {/* CABECERA: Única zona con clic para expandir */}
                ...
                <div style={{ display: 'flex', padding: '15px 5px', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExp && !isFinished && isAdmin && players.filter(p => p.team_id === m.team_a_id).map(p => (
                            <div key={p.id} style={{display:'flex', alignItems:'center', marginBottom:12}}>
                                <button onClick={() => handleGoal(m.id, p.id, m.team_a_id, 'team_a_goals', 'add')} style={{padding:'16px 4px', flex:1, fontSize:13, fontWeight:'bold', borderRadius:10, background: getGoalsInMatch(p.id, m.id) > 0 ? '#ffc107' : '#f0f4ff', border:'1px solid #ccd'}}>⚽ {p.name} {getGoalsInMatch(p.id, m.id) > 0 ? `(${getGoalsInMatch(p.id, m.id)})` : ''}</button>
                                <button onClick={() => handleGoal(m.id, p.id, m.team_a_id, 'team_a_goals', 'remove')} style={{marginLeft:5, color:'red', border:'none', background:'none', fontSize:30}}>-</button>
                            </div>
                        ))}
                    </div>
                    <div style={{ flex: '0 0 28%', textAlign: 'center', paddingTop: '10px' }}>
                        ...
                        {isExp && isAdmin && (
                            <button onClick={() => isFinished ? setEditingMatch({...m}) : axios.put(`${API_URL}/matches/${m.id}`, {...m, played:1}).then(()=>loadData())} style={{marginTop:20, padding:'10px 15px', background: isFinished ? '#ffc107' : 'green', color: isFinished ? '#000' : '#fff', border:'none', borderRadius:10, fontSize:12, fontWeight:'bold'}}>
                    ...
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExp && !isFinished && isAdmin && players.filter(p => p.team_id === m.team_b_id).map(p => (
                            <div key={p.id} style={{display:'flex', alignItems:'center', marginBottom:12}}>
                                <button onClick={() => handleGoal(m.id, p.id, m.team_b_id, 'team_b_goals', 'remove')} style={{marginRight:5, color:'red', border:'none', background:'none', fontSize:30}}>-</button>
                                <button onClick={() => handleGoal(m.id, p.id, m.team_b_id, 'team_b_goals', 'add')} style={{padding:'16px 4px', flex:1, fontSize:13, fontWeight:'bold', borderRadius:10, background: getGoalsInMatch(p.id, m.id) > 0 ? '#ffc107' : '#fff0f0'}}> {p.name} {getGoalsInMatch(p.id, m.id) > 0 ? `(${getGoalsInMatch(p.id, m.id)})` : ''} ⚽</button>
                            </div>
                        ))}
                    </div>
                </div>
    ...
    if (loading) return <div style={{ padding: '100px 0', textAlign: 'center' }}>Cargando v3.5.4...</div>;
    ...
    return (
        <div style={{ padding: '0 0 50px 0', fontFamily: 'Arial', maxWidth: '600px', margin: 'auto', background: '#f8f9fa', minHeight: '100vh' }}>
            ...
            <div style={{ padding: '15px' }}>
                {/* 🏆 CUADRO DE PLAYOFFS DINÁMICO (Sincronizado con standings) */}
