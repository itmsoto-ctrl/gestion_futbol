...
const TournamentView = ({ user }) => {
    ...
    const handleReset = async (target) => {
        if (window.prompt(`Código RESET ${target}:`) !== "0209") return alert("Código incorrecto");
        await axios.post(`${API_URL}/reset-tournament/${id}`, { target });
        window.location.reload();
    };

    const activatePhase = async (phase, pairings) => {
        if (window.prompt(`Código activación ${phase}:`) !== "0209") return alert("Código incorrecto");
            ...
            <div style={{ border: '1px solid #ddd', borderRadius: '15px', background: isFinished ? '#e8f5e9' : 'white', marginBottom: '15px', overflow:'hidden', boxShadow:'0 3px 6px rgba(0,0,0,0.05)' }}>
                ...
                <div style={{ display: 'flex', padding: '20px 5px 10px 5px', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ flex: '0 0 28%', textAlign: 'center', paddingTop: '10px' }}>
                        ...
                        {isExp && isAdmin && (
                            <button onClick={(e) => { e.stopPropagation(); isFinished ? setEditingMatch({...m}) : axios.put(`${API_URL}/matches/${m.id}`, {...m, played:1}).then(()=>loadData()); }} style={{marginTop:20, padding:'10px 15px', background: isFinished ? '#ffc107' : '#28a745', color: isFinished ? '#000' : '#fff', border:'none', borderRadius:10, fontSize:12, fontWeight:'bold', width:'100%'}}>
    ...
    if (loading) return <div style={{ padding: '100px 0', textAlign: 'center' }}>Cargando v3.5.5...</div>;
        ...
        <div style={{ padding: '0 0 50px 0', fontFamily: 'Arial', maxWidth: '600px', margin: 'auto', background: '#f8f9fa', minHeight: '100vh' }}>
            ...
            {showResetMenu && (
                <div style={{ background: '#fff', padding: 15, borderBottom: '2px solid #dc3545', display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
                    <button onClick={() => handleReset('all')} style={{ padding: '12px', background: '#dc3545', color: '#fff', borderRadius: 8, fontSize: 11, fontWeight:'bold' }}>TODO</button>
                    <button onClick={() => handleReset('cuartos')} style={{ padding: '12px', background: '#333', color: '#fff', borderRadius: 8, fontSize: 11, fontWeight:'bold' }}>CUARTOS</button>
                    <button onClick={() => handleReset('semifinal')} style={{ padding: '12px', background: '#333', color: '#fff', borderRadius: 8, fontSize: 11, fontWeight:'bold' }}>SEMIS</button>
                    <button onClick={() => handleReset('final')} style={{ padding: '12px', background: '#333', color: '#fff', borderRadius: 8, fontSize: 11, fontWeight:'bold' }}>FINAL</button>
                </div>
            )}
                    ...
                    <div>
                        ...
                        {isAdmin && (
                            <div style={{marginBottom:20}}>
                                ...
                                {qM.length === 4 && qM.every(m=>m.played) && sM.length === 0 && <button onClick={() => activatePhase('semifinal', [{a: getWinner(qM[0]).id, b: getWinner(qM[3]).id, field: 1}, {a: getWinner(qM[1]).id, b: getWinner(qM[2]).id, field: 2}])} style={{width:'100%', padding:18, background:'#28a745', color:'#fff', border:'none', borderRadius:12, fontWeight:'bold', fontSize:16}}>⚡ ACTIVAR SEMIFINALES</button>}
                                {sM.length === 2 && sM.every(m=>m.played) && fM.length === 0 && <button onClick={() => activatePhase('final', [{a: getWinner(sM[0]).id, b: getWinner(sM[1]).id, field: 1}])} style={{width:'100%', padding:18, background:'#28a745', color:'#fff', border:'none', borderRadius:12, fontWeight:'bold', fontSize:16}}>⚡ ACTIVAR GRAN FINAL</button>}
                            </div>
                        )}
            ...
            {editingMatch && (
                <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.85)', zIndex:2000, padding:20, display:'flex', alignItems:'center'}}>
                    <div style={{background:'#fff', width:'100%', padding:20, borderRadius:15}}>
                        <h4 style={{marginTop:0}}>Edición Manual (v3.5.5)</h4>
