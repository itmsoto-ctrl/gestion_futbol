...
const TournamentView = ({ user }) => {
    ...
    const handleActivate = async (phase) => {
        ...
        if (phase === 'semifinal') pairings = [{a: getWinnerId(qM[0]), b: getWinnerId(qM[3]), field:1}, {a: getWinnerId(qM[1]), b: getWinnerId(qM[2]), field:2}];
        if (phase === 'final') pairings = [{a: getWinnerId(sM[0]), b: getWinnerId(sM[1]), field:1}];
                ...
                <div style={{ display: 'flex', padding: '20px 5px 10px 5px', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExp && isAdmin && !isFin && players.filter(p => p.team_id === m.team_a_id).map(p => {
                            ...
                            return (
                                <button key={p.id} onClick={(e) => handleGoal(e, m.id, p.id, m.team_a_id, 'team_a_goals', 'add')} style={{padding:'14px 4px', width:'95%', fontSize:13, fontWeight:'bold', borderRadius:10, background: g > 0 ? '#ffc107' : '#f0f4ff', marginBottom:8, border:'1px solid #ccc'}}>
                                    ⚽ {p.name} {g > 0 ? `(${g})` : ''}
                                </button>
                            );
                        })}
                    ...
                    <div style={{ flex: '0 0 28%', textAlign: 'center', paddingTop: '10px' }}>
                        ...
                        {isExp && isAdmin && (
                            <button onClick={(e) => { e.stopPropagation(); isFin ? setEditingMatch({...m}) : axios.put(`${API_URL}/matches/${m.id}`, {...m, played:1}).then(()=>loadData()); }} 
                    ...
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExp && isAdmin && !isFin && players.filter(p => p.team_id === m.team_b_id).map(p => {
                            const g = getGoalsInMatch(p.id, m.id);
                            return (
                                <button key={p.id} onClick={(e) => handleGoal(e, m.id, p.id, m.team_b_id, 'team_b_goals', 'add')} style={{padding:'14px 4px', width:'95%', fontSize:13, fontWeight:'bold', borderRadius:10, background: g > 0 ? '#ffc107' : '#f0f4ff', marginBottom:8, border:'1px solid #ccc'}}>
                                    {p.name} {g > 0 ? `(${g})` : ''} ⚽
                                </button>
                            );
                        })}
