...
const TournamentView = ({ user }) => {
    ...
    const handleActivate = async (phase) => {
        ...
        if (phase === 'cuartos') {
            ...
            pairings = [{a: getWinner(qM[0]).id, b: getWinner(qM[3]).id, field: 1}, {a: getWinner(qM[1]).id, b: getWinner(qM[2]).id, field: 2}];
        } else if (phase === 'final') {
            pairings = [{a: getWinner(sM[0]).id, b: getWinner(sM[1]).id, field: 1}];
        }
        ...
        return (
            <div style={{ border: '1px solid #ddd', borderRadius: '15px', background: isFin ? '#e8f5e9' : 'white', marginBottom: '15px', overflow:'hidden', boxShadow:'0 3px 6px rgba(0,0,0,0.05)' }}>
                <div onClick={() => isAdmin && setExpandedMatchId(isExp ? null : m.id)}
                     style={{ background: isFin ? '#c8e6c9' : '#f8f9fa', padding: '12px 15px', fontSize: '13px', borderBottom: '1px solid #eee', display:'flex', justifyContent:'space-between', fontWeight:'bold', cursor:'pointer' }}>
                ...
                <div style={{ display: 'flex', padding: '20px 5px', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExp && !isFin && isAdmin && players.filter(p => p.team_id === m.team_a_id).map(p => (
                            <div key={p.id} style={{display:'flex', alignItems:'center', marginBottom:12}}>
                                <button onClick={(e) => { e.stopPropagation(); handleGoal(m.id, p.id, m.team_a_id, 'team_a_goals', 'add'); }} style={{padding:'16px 4px', flex:1, fontSize:13, fontWeight:'bold', borderRadius:10, background: getGoalsInMatch(p.id, m.id) > 0 ? '#ffc107' : '#f0f4ff', border:'1px solid #ccd'}}>⚽ {p.name} {getGoalsInMatch(p.id, m.id) > 0 ? `(${getGoalsInMatch(p.id, m.id)})` : ''}</button>
                    ...
                    <div style={{ flex: '0 0 28%', textAlign: 'center', paddingTop: '10px' }}>
                        ...
                        {isExp && isAdmin && (
                            <button onClick={(e) => { e.stopPropagation(); isFin ? setEditingMatch({...m}) : axios.put(`${API_URL}/matches/${m.id}`, {...m, played:1}).then(()=>loadData()); }} style={{marginTop:20, padding:'10px 15px', background: isFin ? '#ffc107' : '#28a745', color: isFin ? '#000' : '#fff', border:'none', borderRadius:10, fontSize:12, fontWeight:'bold'}}>
                    ...
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExp && !isFin && isAdmin && players.filter(p => p.team_id === m.team_b_id).map(p => (
                            <div key={p.id} style={{display:'flex', alignItems:'center', marginBottom:12}}>
                                ...
                                <button onClick={(e) => { e.stopPropagation(); handleGoal(m.id, p.id, m.team_b_id, 'team_b_goals', 'add'); }} style={{padding:'16px 4px', flex:1, fontSize:13, fontWeight:'bold', borderRadius:10, background: getGoalsInMatch(p.id, m.id) > 0 ? '#ffc107' : '#f0f4ff'}}> {p.name} {getGoalsInMatch(p.id, m.id) > 0 ? `(${getGoalsInMatch(p.id, m.id)})` : ''} ⚽</button>
                            </div>
                        ))}
                    </div>
                </div>
        ...
        <div style={{ padding: '0 0 50px 0', fontFamily: 'Arial', maxWidth: '600px', margin: 'auto', background: '#f8f9fa', minHeight: '100vh' }}>
            ...
            {showResetMenu && (
                <div style={{ background: '#fff', padding: 15, borderBottom: '2px solid #dc3545', display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
                    <button onClick={() => handleReset('all')} style={{ padding: 10, background: '#dc3545', color: '#fff', borderRadius: 5, fontSize: 10 }}>RESET TODO</button>
