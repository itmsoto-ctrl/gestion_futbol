    ...
    const MatchCard = ({ m }) => {
        ...
        return (
            <div onClick={() => isAdmin && setExpandedMatchId(isExpanded ? null : m.id)}
                ...
                <div style={{ background: '#f1f3f5', padding: '10px', fontSize: '10px', display:'flex', justifyContent:'space-between' }}>
                    ...
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExpanded && isAdmin && players.filter(p => p.team_id === m.team_a_id).map(p => (
                            <div key={p.id} style={{marginBottom:10, display:'flex', alignItems:'center', justifyContent:'center'}}>
                                <button onClick={(e) => { e.stopPropagation(); addGoal(m.id, p.id, m.team_a_id, 'team_a_goals'); }} style={{padding:'14px 4px', width:'80%', fontSize:13, fontWeight:'bold', borderRadius:10, background:'#f0f4ff', border:'1px solid #ccd'}}>⚽ {p.name}</button>
                    ...
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExpanded && isAdmin && players.filter(p => p.team_id === m.team_b_id).map(p => (
                            <div key={p.id} style={{marginBottom:10, display:'flex', alignItems:'center', justifyContent:'center'}}>
                                ...
                                <button onClick={(e) => { e.stopPropagation(); addGoal(m.id, p.id, m.team_b_id, 'team_b_goals'); }} style={{padding:'14px 4px', width:'80%', fontSize:13, fontWeight:'bold', borderRadius:10, background:'#fff0f0', border:'1px solid #dcc'}}>{p.name} ⚽</button>
                            </div>
                        ))}
                    </div>
                </div>
                {isAdmin && isExpanded && <div style={{textAlign:'center', paddingBottom:10}}><button onClick={(e) => { e.stopPropagation(); setEditingMatch({...m}); }} style={{fontSize:10, padding:'5px 10px'}}>✏️ EDITAR PARTIDO</button></div>}
            </div>
        );
    };
    ...
    return (
        <div style={{ padding: '0 0 50px 0', fontFamily: 'Arial', maxWidth: '600px', margin: 'auto', background: '#f8f9fa', minHeight: '100vh' }}>
            ...
            <div style={{ padding: '15px' }}>
                {/* 1. CUADRO DE ELIMINATORIAS DINÁMICO REAL-TIME */}
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '5px' }}>
                            <div style={{ flex: 1.2 }}>
                                {[ [1,8], [2,7], [3,6], [4,5] ].map((pair, i) => {
                                    const m = qMatches[i];
                                    const tA = m ? {name: m.team_a_name} : getTeamByRank(pair[0]);
                                    const tB = m ? {name: m.team_b_name} : getTeamByRank(pair[1]);
                                    return (
                                        <div key={i} style={{ background: '#f8f9fa', marginBottom: '5px', padding: '5px', borderRadius: '5px', fontSize: '9px', border:'1px solid #eee' }}>
                                            <div style={{display:'flex', justifyContent:'space-between'}}><span>{tA.name}</span><b>{m?.team_a_goals ?? ''}</b></div>
                                            <div style={{display:'flex', justifyContent:'space-between', borderTop:'1px solid #fff'}}><span>{tB.name}</span><b>{m?.team_b_goals ?? ''}</b></div>
                                        </div>
                                    );
                                })}
                            </div>
                            <div style={{ flex: 1, display:'flex', flexDirection:'column', justifyContent:'space-around', fontSize:8, textAlign:'center' }}>
                                {[0,1].map(i => {
                                    const m = sMatches[i];
                                    const tA = m ? m.team_a_name : getWinner(qMatches[i*2]).name;
                                    const tB = m ? m.team_b_name : getWinner(qMatches[i*2+1]).name;
                                    return (
                                        <div key={i} style={{border:'1px dashed #ccc', padding:5, borderRadius:5}}><div>{tA}</div><div>{tB}</div></div>
                                    );
                                })}
                            </div>
                    ...
                    <div>
                        ...
                        {isAdmin && (
                            <div style={{marginBottom:20}}>
                                {standingsList.length === 8 && qMatches.length === 0 && <button onClick={() => activatePhase('cuartos', [{a: standingsList[0].id, b: standingsList[7].id, field: 1}, {a: standingsList[1].id, b: standingsList[6].id, field: 2}, {a: standingsList[2].id, b: standingsList[5].id, field: 1}, {a: standingsList[3].id, b: standingsList[4].id, field: 2}])} style={{width:'100%', padding:15, background:'#28a745', color:'#fff', border:'none', borderRadius:10, fontWeight:'bold'}}>⚡ ACTIVAR CUARTOS</button>}
                                {qMatches.length === 4 && qMatches.every(m => m.played) && sMatches.length === 0 && <button onClick={() => activatePhase('semi', [{a: (qMatches[0].team_a_goals > qMatches[0].team_b_goals ? qMatches[0].team_a_id : qMatches[0].team_b_id), b: (qMatches[3].team_a_goals > qMatches[3].team_b_goals ? qMatches[3].team_a_id : qMatches[3].team_b_id), field: 1}, {a: (qMatches[1].team_a_goals > qMatches[1].team_b_goals ? qMatches[1].team_a_id : qMatches[1].team_b_id), b: (qMatches[2].team_a_goals > qMatches[2].team_b_goals ? qMatches[2].team_a_id : qMatches[2].team_b_id), field: 2}])} style={{width:'100%', padding:15, background:'#28a745', color:'#fff', border:'none', borderRadius:10, fontWeight:'bold'}}>⚡ ACTIVAR SEMIFINALES</button>}
                                {sMatches.length === 2 && sMatches.every(m => m.played) && fMatches.length === 0 && <button onClick={() => activatePhase('final', [{a: (sMatches[0].team_a_goals > sMatches[0].team_b_goals ? sMatches[0].team_a_id : sMatches[0].team_b_id), b: (sMatches[1].team_a_goals > sMatches[1].team_b_goals ? sMatches[1].team_a_id : sMatches[1].team_b_id), field: 1}])} style={{width:'100%', padding:15, background:'#28a745', color:'#fff', border:'none', borderRadius:10, fontWeight:'bold'}}>⚡ ACTIVAR GRAN FINAL</button>}
                            </div>
                        )}
                        ...
                        {matches.filter(m => m.phase.toLowerCase().includes('grupo') || m.phase.toLowerCase().includes('liga')).map(m => <MatchCard key={m.id} m={m} />)}
            ...
            {editingMatch && (
                <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.85)', zIndex:2000, padding:20, display:'flex', alignItems:'center'}}>
                    <div style={{background:'#fff', width:'100%', padding:20, borderRadius:15}}>
                        ...
                        <input placeholder="Árbitro" value={editingMatch.referee} onChange={e => setEditingMatch({...editingMatch, referee: e.target.value})} style={{width:'100%', padding:12, marginBottom:15}} />
                        <button onClick={() => handleSaveMatch(editingMatch)} style={{width:'100%', padding:15, background:'green', color:'#fff', borderRadius:10, border:'none'}}>GUARDAR</button>
                        <button onClick={() => setEditingMatch(null)} style={{width:'100%', padding:12, background:'#eee', marginTop:10, borderRadius:10, width:'100%', border:'none'}}>CERRAR</button>
                    </div>
                </div>
            )}
        </div>
