...
const TournamentView = ({ user }) => {
            ...
            >
                ...
                <div style={{ display: 'flex', padding: '15px 5px', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ flex: '0 0 28%', textAlign: 'center', paddingTop: '10px' }}>
                        ...
                        {isExp && isAdmin && (
                            <button onClick={(e) => { e.stopPropagation(); axios.put(`${API_URL}/matches/${m.id}`, {...m, played: isFin ? 0 : 1}).then(()=>loadData()); }} style={{marginTop:20, padding:'10px 15px', background: isFin ? '#ffc107' : '#28a745', color: isFin ? '#000' : '#fff', border:'none', borderRadius:10, fontSize:12, fontWeight:'bold', width:'100%' }}>
    ...
    if (loading) return <div style={{ padding: '100px 0', textAlign: 'center' }}>Cargando v3.7.9...</div>;
