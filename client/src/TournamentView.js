...
const TournamentView = ({ user }) => {
    ...
    const standings = useMemo(() => {
        ...
        matches.forEach(m => {
            if (m.played && (m.phase.includes('grupo') || m.phase.includes('liga'))) {
    ...
    const handleReset = async (target) => {
        if (window.prompt(`Escribe el código para resetear ${target}:`) !== "0209") return alert("Código Incorrecto");
        await axios.post(`${API_URL}/reset-tournament/${id}`, { target });
        window.location.reload();
    };

    const activatePhase = async (phase, pairings) => {
        if (!window.confirm(`¿Confirmas que la fase anterior ha terminado? Se fijarán los cruces de ${phase}.`)) return;
    ...
    const MatchCard = ({ m }) => {
        ...
        return (
            <div onClick={() => isAdmin && setExpandedMatchId(isExp ? null : m.id)}
                ...
                <div style={{ background: isFinished ? '#c8e6c9' : '#f8f9fa', padding: '12px 15px', fontSize: '12px', borderBottom: '1px solid #eee', display:'flex', justifyContent:'space-between', fontWeight:'bold' }}>
                ...
                <div style={{ display: 'flex', padding: '15px 5px', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExp && !isFinished && players.filter(p => p.team_id === m.team_a_id).map(p => {
                            const g = getGoalsInMatch(p.id, m.id);
                            return (
                                <div key={p.id} style={{display:'flex', alignItems:'center', marginBottom:12}}>
                                    <button onClick={(e) => { e.stopPropagation(); handleGoal(m.id, p.id, m.team_a_id, 'team_a_goals', 'add'); }} style={{padding:'16px 4px', flex:1, fontSize:13, fontWeight:'bold', borderRadius:10, background: g > 0 ? '#ffc107' : '#f0f4ff', border:'1px solid #ccd'}}>⚽ {p.name} {g > 0 ? `(${g})` : ''}</button>
                                    <button onClick={(e) => { e.stopPropagation(); handleGoal(m.id, p.id, m.team_a_id, 'team_a_goals', 'remove'); }} style={{marginLeft:8, color:'red', border:'none', background:'none', fontSize:30}}>×</button>
                                </div>
                            );
                        })}
                    </div>
                    <div style={{ flex: '0 0 28%', textAlign: 'center', paddingTop: '10px' }}>
                        ...
                        {isExp && isAdmin && (
                            <button onClick={(e) => { e.stopPropagation(); isFinished ? setEditingMatch({...m}) : axios.put(`${API_URL}/matches/${m.id}`, {...m, played:1}).then(()=>loadData()); }} style={{marginTop:20, padding:'10px 15px', background: isFinished ? '#ffc107' : '#28a745', color: isFinished ? '#000' : '#fff', border:'none', borderRadius:10, fontSize:12, fontWeight:'bold'}}>
                    ...
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExp && !isFinished && players.filter(p => p.team_id === m.team_b_id).map(p => {
                            const g = getGoalsInMatch(p.id, m.id);
                            return (
                                <div key={p.id} style={{display:'flex', alignItems:'center', marginBottom:12}}>
                                    <button onClick={(e) => { e.stopPropagation(); handleGoal(m.id, p.id, m.team_b_id, 'team_b_goals', 'remove'); }} style={{marginRight:8, color:'red', border:'none', background:'none', fontSize:30}}>×</button>
                                    <button onClick={(e) => { e.stopPropagation(); handleGoal(m.id, p.id, m.team_b_id, 'team_b_goals', 'add'); }} style={{padding:'16px 4px', flex:1, fontSize:13, fontWeight:'bold', borderRadius:10, background: g > 0 ? '#ffc107' : '#f0f4ff'}}>{p.name} {g > 0 ? `(${g})` : ''} ⚽</button>
                                </div>
                            );
                        })}
                    </div>
    ...
    const gM = matches.filter(m => m.phase.toLowerCase().includes('grupo'));
    ...
    return (
        <div style={{ padding: '0 0 50px 0', fontFamily: 'Arial', maxWidth: '600px', margin: 'auto', background: '#f8f9fa', minHeight: '100vh' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#000', padding: '15px 20px', color: 'white', position: 'sticky', top: 0, zIndex: 1000 }}>
                ...
                <div>
                    {isAdmin && <button onClick={() => setShowResetMenu(!showResetMenu)} style={{background:'#dc3545', color:'#fff', border:'none', padding:'8px 12px', borderRadius:8, fontSize:11, marginRight:10, fontWeight:'bold'}}>RESET ⚙️</button>}
            ...
            {showResetMenu && (
                <div style={{ background: '#fff', padding: 15, borderBottom: '2px solid #dc3545', display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
                    <button onClick={() => handleReset('all')} style={{ padding: '10px', background: '#dc3545', color: '#fff', borderRadius: 5, fontSize: 10 }}>RESET TODO</button>
                    <button onClick={() => handleReset('cuartos')} style={{ padding: '10px', background: '#333', color: '#fff', borderRadius: 5, fontSize: 10 }}>RESET CUARTOS</button>
                    <button onClick={() => handleReset('semi')} style={{ padding: '10px', background: '#333', color: '#fff', borderRadius: 5, fontSize: 10 }}>RESET SEMIS</button>
                    <button onClick={() => handleReset('final')} style={{ padding: '10px', background: '#333', color: '#fff', borderRadius: 5, fontSize: 10 }}>RESET FINAL</button>
                    <button onClick={() => setShowResetMenu(false)} style={{ padding: '10px', background: '#eee', borderRadius: 5, fontSize: 10 }}>CERRAR</button>
                </div>
            )}

            <div style={{ padding: '15px' }}>
                {/* CUADRO PROYECTADO SIEMPRE BASADO EN STANDINGS */}
                            ...
                            <div style={{ flex: 1.2 }}>
                                {[ [1,8], [2,7], [3,6], [4,5] ].map((pair, i) => (
                                    <div key={i} style={{ background: '#f8f9fa', marginBottom: '5px', padding: '5px', borderRadius: '5px', border: '1px solid #eee', fontSize: '9px' }}>
                                        <div style={{display:'flex', justifyContent:'space-between'}}><span>{qM[i] ? qM[i].team_a_name : (standings[pair[0]-1]?.name || pair[0]+'º')}</span><b>{qM[i]?.team_a_goals ?? ''}</b></div>
                                        <div style={{display:'flex', justifyContent:'space-between', borderTop:'1px solid #fff'}}><span>{qM[i] ? qM[i].team_b_name : (standings[pair[1]-1]?.name || pair[1]+'º')}</span><b>{qM[i]?.team_b_goals ?? ''}</b></div>
                                    </div>
                                ))}
                            </div>
                ...
                {showTable ? (
                    ...
                    <div>
                        ...
                        {gM.map(m => <MatchCard key={m.id} m={m} />)}
                        ...
                        {qM.length > 0 ? qM.map(m => <MatchCard key={m.id} m={m} />) : (
                            isAdmin && gM.length > 0 && gM.every(m=>m.played) && 
                            <button onClick={() => activatePhase('cuartos', [{a: standings[0].id, b: standings[7].id, field:1}, {a: standings[1].id, b: standings[6].id, field:2}, {a: standings[2].id, b: standings[5].id, field:1}, {a: standings[3].id, b: standings[4].id, field:2}])} style={{width:'100%', padding:15, background:'#28a745', color:'#fff', borderRadius:10, border:'none', fontWeight:'bold'}}>⚡ GENERAR CUARTOS</button>
                        )}
                        ...
                        {sM.length > 0 ? sM.map(m => <MatchCard key={m.id} m={m} />) : (
                            isAdmin && qM.length === 4 && qM.every(m=>m.played) && 
                            <button onClick={() => activatePhase('semifinal', [{a: getWinner(qM[0]).id, b: getWinner(qM[3]).id, field:1}, {a: getWinner(qM[1]).id, b: getWinner(qM[2]).id, field:2}])} style={{width:'100%', padding:15, background:'#28a745', color:'#fff', borderRadius:10, border:'none', fontWeight:'bold'}}>⚡ GENERAR SEMIFINALES</button>
                        )}
                        ...
                        {fM.length > 0 ? fM.map(m => <MatchCard key={m.id} m={m} />) : (
                            isAdmin && sM.length === 2 && sM.every(m=>m.played) && 
                            <button onClick={() => activatePhase('final', [{a: getWinner(sM[0]).id, b: getWinner(sM[1]).id, field:1}])} style={{width:'100%', padding:15, background:'#28a745', color:'#fff', borderRadius:10, border:'none', fontWeight:'bold'}}>⚡ GENERAR GRAN FINAL</button>
                        )}
                    </div>
                )}
            ...
            {editingMatch && (
                <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.85)', zIndex:2000, padding:20, display:'flex', alignItems:'center'}}>
                    <div style={{background:'#fff', width:'100%', padding:20, borderRadius:15}}>
                        ...
                        <input type="number" value={editingMatch.team_a_goals} onChange={e => setEditingMatch({...editingMatch, team_a_goals: parseInt(e.target.value)})} style={{width:'90%', padding:12, marginBottom:10}} />
                        <label style={{fontSize:11}}>Goles {editingMatch.team_b_name}:</label>
                        <input type="number" value={editingMatch.team_b_goals} onChange={e => setEditingMatch({...editingMatch, team_b_goals: parseInt(e.target.value)})} style={{width:'90%', padding:12, marginBottom:10}} />
