...
const TournamentView = ({ user }) => {
    ...
    const MatchCard = ({ m }) => {
        ...
        return (
            <div style={{ border: '1px solid #ddd', borderRadius: '15px', background: isFinished ? '#e8f5e9' : 'white', marginBottom: '15px', overflow:'hidden', boxShadow:'0 3px 6px rgba(0,0,0,0.05)' }}>
                {/* CABECERA GRIS: Única zona para expandir/cerrar */}
                <div 
                ...
                <div style={{ display: 'flex', padding: '20px 5px 10px 5px', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExp && isAdmin && !isFinished && players.filter(p => p.team_id === m.team_a_id).map(p => {
                            ...
                            return (
                                <button key={p.id} onClick={(e) => { e.stopPropagation(); handleGoal(m.id, p.id, m.team_a_id, 'team_a_goals', 'add'); }} style={{padding:'14px 4px', width:'95%', fontSize:12, fontWeight:'bold', borderRadius:10, background: g > 0 ? '#ffc107' : '#f0f4ff', marginBottom:8, border:'1px solid #ccc'}}>
                                    ⚽ {p.name} {g > 0 ? `(${g})` : ''}
                                </button>
                            );
                        })}
                    ...
                    <div style={{ flex: '0 0 28%', textAlign: 'center', paddingTop: '10px' }}>
                        ...
                        {isExp && isAdmin && !isFinished && <div style={{fontSize:9, marginTop:15, color:'#aaa'}}>Pulsa un jugador para anotar</div>}
                    </div>
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExp && isAdmin && !isFinished && players.filter(p => p.team_id === m.team_b_id).map(p => {
                            const g = getGoalsInMatch(p.id, m.id);
                            return (
                                <button key={p.id} onClick={(e) => { e.stopPropagation(); handleGoal(m.id, p.id, m.team_b_id, 'team_b_goals', 'add'); }} style={{padding:'14px 4px', width:'95%', fontSize:12, fontWeight:'bold', borderRadius:10, background: g > 0 ? '#ffc107' : '#f0f4ff', marginBottom:8, border:'1px solid #ccc'}}>
                                    {p.name} {g > 0 ? `(${g})` : ''} ⚽
                                </button>
                            );
                        })}
    ...
    if (loading) return <div style={{ padding: '100px 0', textAlign: 'center' }}>Cargando v3.6...</div>;
