...
const TournamentView = ({ user }) => {
    ...
    const loadData = useCallback(async () => {
        try {
            ...
            if (resTourneys.status === 'fulfilled') setTournamentInfo(resTourneys.value.data.find(t => t.id === parseInt(id)));

        } catch (error) { console.error("Error en carga:", error); }
    ...
    const handleGoal = async (e, mId, pId, tId, side, action) => {
        e.stopPropagation(); // BLOQUEA EL CIERRE DE LA CAJA
                        ...
                        {isExp && isAdmin && !isFinished && players.filter(p => p.team_id === m.team_a_id).map(p => {
                            ...
                            return (
                                <div key={p.id} style={{display:'flex', alignItems:'center', marginBottom:12}}>
                                    <button onClick={(e) => handleGoal(e, m.id, p.id, m.team_a_id, 'team_a_goals', 'add')} style={{padding:'16px 4px', flex:1, fontSize:13, fontWeight:'bold', borderRadius:10, background: g > 0 ? '#ffc107' : '#f0f4ff', border:'1px solid #ccd'}}>⚽ {p.name} {g > 0 ? `(${g})` : ''}</button>
                                    <button onClick={(e) => handleGoal(e, m.id, p.id, m.team_a_id, 'team_a_goals', 'remove')} style={{marginLeft:8, color:'red', background:'none', border:'none', fontSize:30}}>-</button>
                                </div>
                            );
                        })}
                    </div>
                        ...
                        {isExp && isAdmin && !isFinished && players.filter(p => p.team_id === m.team_b_id).map(p => {
                            ...
                            return (
                                <div key={p.id} style={{display:'flex', alignItems:'center', marginBottom:12}}>
                                    ...
                                    <button onClick={(e) => handleGoal(e, m.id, p.id, m.team_b_id, 'team_b_goals', 'add')} style={{padding:'16px 4px', flex:1, fontSize:13, fontWeight:'bold', borderRadius:10, background: g > 0 ? '#ffc107' : '#f0f4ff'}}> {p.name} {g > 0 ? `(${g})` : ''} ⚽</button>
                                </div>
                            );
                        })}
                    </div>
    ...
    if (loading) return <div style={{ padding: '100px 0', textAlign: 'center' }}>v3.6.0 - Cargando...</div>;
        ...
        <div style={{ padding: '0 0 50px 0', fontFamily: 'Arial', maxWidth: '600px', margin: 'auto', background: '#f8f9fa', minHeight: '100vh' }}>
            ...
            {showResetMenu && (
                <div style={{ background: '#fff', padding: 15, borderBottom: '2px solid #dc3545', display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
                    <button onClick={() => handleReset('all')} style={{ padding: '10px', background: '#dc3545', color: '#fff', borderRadius: 5 }}>TODO</button>
                    <button onClick={() => handleReset('cuartos')} style={{ padding: '10px', background: '#333', color: '#fff', borderRadius: 5 }}>CUARTOS</button>
                    <button onClick={() => handleReset('semifinal')} style={{ padding: '10px', background: '#333', color: '#fff', borderRadius: 5 }}>SEMIS</button>
                    <button onClick={() => handleReset('final')} style={{ padding: '10px', background: '#333', color: '#fff', borderRadius: 5 }}>FINAL</button>
                </div>
            )}
                    ...
                    <div style={{ background: '#fff', padding: '15px', borderRadius: '15px', marginBottom: '30px', border: '1px solid #eee', boxShadow:'0 4px 10px rgba(0,0,0,0.05)' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '5px' }}>
                            ...
                            <div style={{ flex: 1, display:'flex', flexDirection:'column', justifyContent:'space-around', fontSize:8, textAlign:'center', color:'#999' }}>
                                {[0, 1].map(i => (<div key={i} style={{border:'1px dashed #ccc', padding:5, borderRadius:5}}><div>{sM[i] ? sM[i].team_a_name : (qM[i*2] ? getWinnerName(qM[i*2]) : 'Winner')}</div><div>{sM[i] ? sM[i].team_b_name : (qM[i*2+1] ? getWinnerName(qM[i*2+1]) : 'Winner')}</div></div>))}
                            </div>
                    ...
                    <div>
                        ...
                        {isAdmin && (
                            <div style={{marginBottom:20}}>
                                {allGPlayed && qM.length === 0 && <button onClick={() => axios.post(`${API_URL}/activate-phase/${id}`, {phase:'cuartos', pairings:[{a:standings[0].id, b:standings[7].id, field:1},{a:standings[1].id, b:standings[6].id, field:2},{a:standings[2].id, b:standings[5].id, field:1},{a:standings[3].id, b:standings[4].id, field:2}]}).then(()=>loadData())} style={{width:'100%', padding:18, background:'#28a745', color:'#fff', border:'none', borderRadius:12, fontWeight:'bold'}}>⚡ ACTIVAR CUARTOS</button>}
                                {qM.length === 4 && qM.every(m=>m.played) && sM.length === 0 && <button onClick={() => axios.post(`${API_URL}/activate-phase/${id}`, {phase:'semifinal', pairings:[{a:getWinnerId(qM[0]), b:getWinnerId(qM[3]), field:1},{a:getWinnerId(qM[1]), b:getWinnerId(qM[2]), field:2}]}).then(()=>loadData())} style={{width:'100%', padding:18, background:'#28a745', color:'#fff', border:'none', borderRadius:12, fontWeight:'bold'}}>⚡ ACTIVAR SEMIFINALES</button>}
                                {sM.length >= 2 && sM.every(m=>m.played) && fM.length === 0 && <button onClick={() => axios.post(`${API_URL}/activate-phase/${id}`, {phase:'final', pairings:[{a:getWinnerId(sM[0]), b:getWinnerId(sM[1]), field:1}]}).then(()=>loadData())} style={{width:'100%', padding:18, background:'#28a745', color:'#fff', border:'none', borderRadius:12, fontWeight:'bold', fontSize:16, cursor:'pointer'}}>⚡ ACTIVAR GRAN FINAL</button>}
                            </div>
                        )}
