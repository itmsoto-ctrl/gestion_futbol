...
const TournamentView = ({ user }) => {
    ...
    const handleReset = async () => {
        if (window.prompt("Código RESET Maestro:") !== "0209") return alert("Código incorrecto");
        await axios.post(`${API_URL}/reset-tournament/${id}`);
        window.location.reload();
    };

    const activatePhase = async (phase, pairings) => {
        if (window.prompt("Código Activación:") !== "0209") return alert("Error");
    ...
    const MatchCard = ({ m }) => {
        ...
        return (
            <div onClick={() => isAdmin && setExpandedMatchId(isExp ? null : m.id)}
                 style={{ border: '1px solid #ddd', borderRadius: '15px', background: isFinished ? '#e8f5e9' : 'white', marginBottom: '15px', overflow:'hidden', boxShadow:'0 3px 6px rgba(0,0,0,0.05)' }}>
                    ...
                    <div style={{ flex: '0 0 28%', textAlign: 'center', paddingTop: '10px' }}>
                        ...
                        {isExp && isAdmin && (
                            <button onClick={(e) => { e.stopPropagation(); isFinished ? setEditingMatch({...m}) : axios.put(`${API_URL}/matches/${m.id}`, {...m, played:1}).then(()=>loadData()); }} style={{marginTop:20, padding:'10px 15px', background: isFinished ? '#ffc107' : '#28a745', color: isFinished ? '#000' : '#fff', border:'none', borderRadius:10, fontSize:12, fontWeight:'bold'}}>
                                {isFinished ? '✏️ DESBLOQUEAR / EDITAR' : '✅ FINALIZAR'}
                            </button>
                        )}
                    </div>
                    <div style={{ flex: '1 1 35%', textAlign: 'center' }}>
                        ...
                        {isExp && !isFinished && players.filter(p => p.team_id === m.team_b_id).map(p => {
                            ...
                            return (
                                <div key={p.id} style={{display:'flex', alignItems:'center', marginBottom:12}}>
                                    ...
                                    <button onClick={(e) => { e.stopPropagation(); handleGoal(m.id, p.id, m.team_b_id, 'team_b_goals', 'add'); }} style={{padding:'16px 4px', flex:1, fontSize:13, fontWeight:'bold', borderRadius:10, background: g > 0 ? '#ffc107' : '#fff0f0', border:'1px solid #dcc'}}>{p.name} {g > 0 ? `(${g})` : ''} ⚽</button>
                                </div>
                            );
                        })}
                    </div>
            ...
            {editingMatch && (
                <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.85)', zIndex:2000, padding:20, display:'flex', alignItems:'center'}}>
                    <div style={{background:'#fff', width:'100%', padding:20, borderRadius:15}}>
                        <h4 style={{marginTop:0}}>Editar Partido (v3.4)</h4>
                        ...
                        <button onClick={() => { axios.put(`${API_URL}/matches/${editingMatch.id}`, {...editingMatch, played: 1}).then(()=> {setEditingMatch(null); loadData();}) }} style={{width:'100%', padding:15, background:'green', color:'#fff', borderRadius:10, border:'none', fontWeight:'bold'}}>GUARDAR Y BLOQUEAR</button>
                        <button onClick={() => setEditingMatch(null)} style={{width:'100%', padding:12, background:'#eee', border:'none', borderRadius:10, marginTop:10}}>CERRAR</button>
                    </div>
                </div>
            )}
        </div>
