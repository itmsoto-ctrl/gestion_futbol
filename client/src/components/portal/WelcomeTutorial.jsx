...
const WelcomeTutorial = ({ user, onFinish }) => {
    ...
    return (
        <div className="fixed inset-0 z-[200] bg-zinc-950 text-white flex flex-col items-center justify-start pt-16 px-6 pb-24 font-sans overflow-hidden italic">
            ...
            <AnimatePresence mode="wait">
                ...
                >
                    ...
                    <div className="flex-1 w-full overflow-y-auto space-y-6 px-2 pb-4 scrollbar-hide">
            ...
            <div className="fixed bottom-10 left-0 right-0 px-6 flex items-center justify-between max-w-md mx-auto w-full z-10">
                <button onClick={prev} className={`p-4 rounded-full border border-white/10 bg-white/5 active:scale-90 transition-all ${currentStep === 1 ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}><ArrowLeft size={24} /></button>
                <div className="flex gap-2">
                    {[1, 2, 3, 4].map(s => (
                        <div key={s} className={`h-1.5 rounded-full transition-all duration-500 ${s === currentStep ? 'bg-lime-400 w-6' : 'bg-white/10 w-2'}`} />
                    ))}
                </div>
                <button onClick={next} className="bg-lime-400 text-black p-4 rounded-full shadow-[0_10px_25px_rgba(163,230,53,0.3)] active:scale-90 transition-all">{currentStep === 4 ? <Check size={24} strokeWidth={3} /> : <ArrowRight size={24} strokeWidth={3} />}</button>
            </div>
        </div>
    );
};
