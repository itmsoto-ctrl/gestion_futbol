...
const WelcomeTutorial = ({ user, onFinish }) => {
    ...
    const steps = [
        {
            title: `¡HOLA,\n${playerName}!`,
            content: `Has sido oficialmente convocado para disputar la liga ${leagueName}.\n\nDefenderás los colores de ${teamName}.\n\nPrepárate para demostrar tu talento en cada partido.`,
            icon: <Star className="text-lime-400" size={50} />
        },
        {
            title: "TU FICHA FUT",
            image: "/tutorial-card.webp",
            content: "Para que tu cromo luzca profesional, busca un fondo liso para tu selfie.\n\nEs tu imagen ante el resto de la liga, así que asegúrate de que se vea impecable.",
            icon: <Camera className="text-lime-400" size={50} />
        },
        {
            title: "SUBE TUS STATS",
            content: "Tras cada jornada, los capitanes registrarán el resultado oficial.\n\nRecibirás una notificación: entra rápido para indicar tus goles y asistencias.\n\n¡Mira cómo mejora tu puntuación Fut tras cada partido!",
            icon: <Zap className="text-lime-400" size={50} />
        },
        {
            title: "INSTALA VORA",
            content: "Haz este paso una única vez para garantizar la mejor experiencia.\n\nInstala la App en tu pantalla de inicio para recibir alertas de tus partidos y ver tus resultados al instante.",
    ...
    return (
        <div className="fixed inset-0 z-[200] bg-zinc-950 text-white flex flex-col items-center justify-start pt-16 px-6 font-sans italic overflow-hidden">
            ...
            <div className="absolute top-0 left-0 w-full h-1.5 bg-white/5">
                <motion.div className="h-full bg-lime-400 shadow-[0_0_15px_rgba(163,230,53,0.5)]" initial={{ width: 0 }} animate={{ width: `${(currentStep / 4) * 100}%` }} />
            </div>

            <AnimatePresence mode="wait">
                <motion.div key={currentStep} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="flex-1 max-w-md w-full flex flex-col items-center text-center overflow-hidden">
                    <div className="mb-4 drop-shadow-[0_0_15px_rgba(163,230,53,0.3)]">{steps[currentStep-1].icon}</div>
                    <div className="flex-1 w-full overflow-y-auto space-y-6 px-2 pb-10 scrollbar-hide">
                        ...
                        {steps[currentStep-1].image && <motion.img initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} src={steps[currentStep-1].image} className="h-44 mx-auto rounded-2xl border border-white/5" alt="FUT Card" />}
            ...
            <div className="fixed bottom-10 left-0 right-0 px-8 flex items-center justify-between max-w-md mx-auto w-full z-10">
                <button onClick={prev} className={`p-4 rounded-full bg-white/5 active:scale-90 transition-all ${currentStep === 1 ? 'opacity-0' : 'opacity-100'}`}><ArrowLeft size={24} /></button>
                ...
                <button onClick={next} className="bg-lime-400 text-black p-4 rounded-full shadow-[0_10px_25px_rgba(163,230,53,0.3)] active:scale-90 transition-all">{currentStep === 4 ? <Check size={24} /> : <ArrowRight size={24} />}</button>
            </div>
        </div>
    );
};
