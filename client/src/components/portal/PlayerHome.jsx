...
// Recuperamos el Onboarding original para la edición
...
const PlayerHome = () => {
    ...
    // Recuperamos tu estado original 'editMode'
            ...
            try {
                ...
                if (data) {
                    ...
                    if (data.tutorial_seen === 0) {
                        ...
                        // Si no tiene foto, activamos el modo edición de ayer
                        setEditMode(true); 
                    }
    ...
    const completeTutorial = async () => {
        ...
        if (!user?.photo_url) setEditMode(true);
    };
    ...
    if (showTutorial) return <WelcomeTutorial user={user} onFinish={completeTutorial} />;
    ...
    return (
        <div className="min-h-screen bg-cover bg-center flex overflow-hidden font-sans relative" style={{ backgroundImage: "url('/bg-home-player.webp')" }}>
            ...
            {/* Si NO estamos en modo edición, mostramos la barra lateral */}
            {!editMode && (
            ...
            <main className="flex-1 flex flex-col items-center justify-start relative px-4 overflow-y-auto pt-10">
                ...
                {/* 🎯 ESTA ES TU LÓGICA DE AYER: 
                    Si es Onboarding, dibuja el componente de edición. 
                    Si no, dibuja la tarjeta clickeable. */}
                {editMode ? (
                    ...
                    <>
                        <div onClick={() => { playClick(); setEditMode(true); }} className="cursor-pointer transform scale-[0.6] sm:scale-75 active:scale-95 transition-all drop-shadow-2xl mt-[-90px]">
