...
const PlayerRegistration = () => {
    ...
    if (step === 4) return (
        <div className="min-h-screen bg-[#665C5A] flex flex-col items-center justify-center p-6 text-center animate-in zoom-in duration-700">
            ...
            <h1 className="text-3xl font-black uppercase italic text-white leading-none">Fichaje Estrella <br/><span className="text-lime-400">Confirmado</span></h1>
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-[#665C5A] text-white p-6 flex flex-col items-center overflow-x-hidden">
            ...
            <div className="w-full max-w-sm space-y-6">
                {step === 1 && (
                    <form onSubmit={handleCheckEmail} className="space-y-6 animate-in fade-in slide-in-from-bottom-8">
                ...
                {step === 2 && (
                    <form onSubmit={(e) => {e.preventDefault(); setStep(3);}} className="space-y-4 animate-in slide-in-from-right duration-500">
                        ...
                        <input required type="password" placeholder="CONTRASEÑA" className="w-full glass-input p-5 rounded-[2rem] outline-none font-bold"
                ...
                {/* Paso 3 (Cromo) igual que antes pero sobre el fondo gris limpio */}
                {step === 3 && (
                    <div className="flex flex-col items-center animate-in zoom-in-95">
                        ...
                        <button 
                            ...
                            className="w-full mt-10 bg-lime-400 text-zinc-950 font-black py-5 rounded-[2.5rem] italic text-xl shadow-xl active:scale-95"
                        >
                            {uploading ? <Loader2 className="animate-spin"/> : "FINALIZAR FICHA"}
                        </button>
                    </div>
                )}
            </div>
