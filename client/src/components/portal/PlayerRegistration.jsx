...
const PlayerRegistration = () => {
    ...
    const [step, setStep] = useState(1); // 1: Email, 2: Auth/Datos, 3: Cromo, 4: Éxito
    ...
    // Cargar email recordado y datos del equipo
    useEffect(() => {
    ...
    // PASO 1: Validar si el email existe
    ...
    // PASO 2: Gestión de Foto (Cloudinary)
    ...
    // PASO FINAL: Registro completo en Base de Datos
    ...
    if (step === 4) return (
        <div className="min-h-screen bg-net flex flex-col items-center justify-center p-6 text-center animate-in zoom-in duration-700">
            <CheckCircle size={100} className="text-lime-400 mb-6 drop-shadow-[0_0_20px_rgba(163,230,53,0.4)]" />
    ...
    return (
        <div className="min-h-screen bg-net text-white font-sans p-6 flex flex-col items-center">
            
            {/* LOGO QUE DIBUJA EL CONTORNO (USANDO CSS .logo-contour-draw) */}
            <div className="mt-6 mb-8 animate-in fade-in zoom-in duration-1000">
                <div className="logo-container">
                    <img 
                        src="/logo-shine.webp" 
                        alt="VORA" 
                        className="logo-contour-draw" 
                    />
                </div>
            </div>

            {/* INFO DEL EQUIPO (SUTIL SOBRE EL GRIS) */}
            {teamInfo && step < 3 && (
                <div className="w-full max-w-sm mb-8 animate-in fade-in slide-in-from-top duration-700">
                    <div className="text-center">
                        <UserCircle2 className="mx-auto text-lime-400 opacity-60 mb-2" size={32} />
                        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-lime-400">Inscripción Oficial</p>
                        <h2 className="text-white text-2xl font-black italic uppercase leading-tight tracking-tighter mt-1">
                            {teamInfo.teamName}
                        </h2>
                    </div>
                </div>
            ...
            {step === 2 && (
                <form onSubmit={(e) => {e.preventDefault(); setStep(3);}} className="w-full max-w-sm space-y-4 animate-in slide-in-from-right duration-500">
                    ...
                    <input required type="password" placeholder="TU CONTRASEÑA" className="w-full glass-input p-5 rounded-[2rem] outline-none font-bold"
            ...
            {/* PASO 3: CARTA FUT TEAM (LA ESTRELLA) */}
            {step === 3 && (
                <div className="w-full max-w-sm flex flex-col items-center animate-in zoom-in-95 duration-500">
                    <div className="relative w-64 h-80 bg-gradient-to-b from-lime-400 to-lime-600 rounded-[2.5rem] p-1 fut-card-shadow">
                        <div className="bg-zinc-950 w-full h-full rounded-[2.3rem] overflow-hidden relative border border-lime-400/30">
                            {photoUrl ? (
                                ...
                                <div {...getRootProps()} className="flex flex-col items-center cursor-pointer p-4 h-full justify-center text-center">
                                    ...
                                    <div className="bg-white/5 p-4 rounded-full mb-3">
                                        <Camera className="text-white opacity-40" size={32} />
                                    </div>
                            ...
                            <div className="absolute bottom-5 left-0 right-0 px-6 text-left bg-gradient-to-t from-black via-black/90 to-transparent pt-4">
                                <p className="text-xl font-black uppercase italic leading-none text-white truncate drop-shadow-lg">
                                    {fullName.split(' ')[0] || 'JUGADOR'}
                                </p>
                                <div className="flex justify-between items-center mt-1">
                                    <span className="text-3xl font-black text-lime-400 italic drop-shadow-lg">{dorsal || '00'}</span>
                                    <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-tighter">{teamInfo?.teamName}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <button 
                        ...
                        className="w-full mt-10 bg-lime-400 text-zinc-950 font-black py-6 rounded-[2.5rem] flex items-center justify-center gap-3 italic text-xl shadow-xl disabled:opacity-20 active:scale-95 transition-all"
                    >
                        {loading || uploading ? <Loader2 className="animate-spin"/> : <>FINALIZAR MI FICHA <CheckCircle size={24}/></>}
                    </button>
                </div>
            )}
        </div>
