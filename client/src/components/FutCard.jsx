...
const FutCard = ({ player, size = "large" }) => {
  ...
  const rating = parseInt(player.rating) || 75; // Bajamos el rating por defecto para el bronce
  ...
  return (
    ...
    >
      {/* 1. VÍDEO DE FONDO (Opcional para bronce, pero lo dejamos por el efecto) */}
      ...
      <motion.div 
          ...
          className="absolute inset-0 z-[6] bg-gradient-to-r from-transparent via-white/10 to-transparent w-full -skew-x-12 pointer-events-none" 
      />

      {/* 🖼️ LA NUEVA TARJETA BRONCE SIN SILUETA */}
      <img src="/bronce.png" alt="Card Bronze" className="w-[350px] h-auto relative z-10 select-none pointer-events-none" />
      
      {/* 📸 FOTO JUGADOR (Ahora sin silueta debajo, se verá perfecta) */}
      {player.photo_url && (
      ...
      <div className="absolute top-[55px] left-[43px] z-20 flex flex-col items-center gap-0 text-amber-900"> {/* Color más oscuro para el bronce */}
        ...
        <div className="h-[2px] w-12 bg-amber-900/30 my-2" /> 
      ...
      <div className="absolute top-[278px] left-0 w-full text-center z-30 px-2">
        <span className="text-amber-950 text-[40px] font-bold uppercase italic tracking-[-0.04em] leading-none"
      ...
      {/* 📊 STATS */}
      <div className="absolute top-[345px] left-0 w-full flex justify-center items-center z-20 px-10 text-amber-900">
        ...
        <div className="w-[1.5px] h-20 bg-amber-900/20 mx-1" />
