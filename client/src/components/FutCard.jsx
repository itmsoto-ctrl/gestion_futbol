...
const RatingCounter = ({ targetValue, onComplete }) => {
  ...
  useEffect(() => {
    ...
    playScore(0.3); // ✅ Sonido durante la subida
...
const FutCard = ({ player, isFlipped, onFlip, children }) => {
  ...
  return (
    <div className="relative select-none" style={{ width: '350px', height: '504px', perspective: "2000px" }}>
      <motion.div
        animate={{ 
          rotateY: isFlipped ? 180 : [-8, 8, -8], // ✅ Balanceo más elegante
          rotateX: [2, -2, 2],
          y: [0, -6, 0] // ✅ Efecto flotante
        }}
        transition={{ 
          rotateY: isFlipped ? { duration: 0.8 } : { duration: 7, repeat: Infinity, ease: "easeInOut" },
      ...
      >
        <div className="absolute inset-0 w-full h-full rounded-[45px] overflow-hidden shadow-2xl" style={{ backfaceVisibility: "hidden" }}>
          ...
          {/* FOTO: Rostro un poco más pequeño */}
          {player?.photo_url && (
            <div className="absolute top-[38px] left-[125px] w-[200px] h-[245px] z-[15] pointer-events-none"
              style={{
