...
const RatingCounter = ({ targetValue, onComplete }) => {
  ...
  // 🛡️ Flag global para que solo anime la primera vez en la sesión
  const hasAlreadyAnimated = useRef(sessionStorage.getItem('rating_animated'));

  useEffect(() => {
    if (hasAlreadyAnimated.current) {
      setDisplayValue(targetValue);
      if (onComplete) onComplete();
      return;
    }

    playScore(0.3); // Sonido de subida
    const controls = animate(0, targetValue, {
      duration: 1.8,
      ease: [0.33, 1, 0.68, 1],
      onUpdate: (value) => setDisplayValue(Math.floor(value)),
      onComplete: () => {
        sessionStorage.setItem('rating_animated', 'true');
        if (onComplete) onComplete();
      }
    });
...
const FutCard = ({ player, isFlipped, onFlip, children }) => {
  ...
  return (
    <div className="relative select-none" style={{ width: '350px', height: '504px', perspective: "2000px" }}>
      <motion.div
        animate={{ 
          rotateY: isFlipped ? 180 : [-7, 7, -7], // ✅ Giro reducido (más elegante)
          rotateX: [2, -2, 2] 
        }}
        transition={{ 
        ...
        <div className="absolute inset-0 w-full h-full rounded-[45px] overflow-hidden shadow-2xl" style={{ backfaceVisibility: "hidden" }}>
          ...
          <div className="absolute top-[60px] left-[45px] z-20 flex flex-col items-center text-[#3a2d0f] font-bold font-oswald text-center">
            ...
            <div className="flex flex-col items-center gap-2 mt-3">
               <img src={`https://flagcdn.com/w80/${player?.country_code || 'es'}.png`} className="w-10 border border-black/5" alt="Flag" />
               <img src={player?.team_logo || '/default-team.png'} className="w-11 h-11 object-contain opacity-80" alt="Club" />
            </div>
          </div>
