...
const LeagueCreator = () => {
  ...
  // --- Lógica de Sedes y Modal ---
  const handleOpenModal = () => {
    const input = document.getElementById('venueSearch');
    if (input.value.trim() !== '') {
      setTempVenue({ ...tempVenue, name: input.value });
      setIsModalOpen(true);
      input.value = ''; 
    }
  };
  ...
  // --- Lógica de Publicación (Diagnóstico incorporado) ---
  const handlePublishLeague = async (generatedSchedule) => {
    ...
    setLoading(true);
    console.log("🚀 Enviando liga al servidor...", { config, scheduleCount: generatedSchedule.length });
    ...
    try {
      ...
      if (response.ok) {
        console.log("✅ Liga creada:", data);
        alert(`¡GOL! Liga creada con éxito.`);
        window.location.href = '/admin/dashboard'; 
      } else {
        console.error("❌ Error del servidor:", data);
        alert(`Error: ${data.message || "No se pudo crear la liga"}`);
      }
    } catch (error) {
      console.error("🔥 Error de conexión:", error);
      alert("No se pudo conectar con el servidor. Verifica que el Mac Mini esté encendido.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-950/90 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 w-full max-w-md p-8 rounded-[2.5rem] space-y-6">
            ...
            <div className="space-y-4">
              <input type="text" value={tempVenue.name} readOnly className="w-full bg-zinc-800/50 rounded-2xl py-4 px-5 text-sm text-zinc-500 border border-zinc-700" />
      ...
      <div className="max-w-7xl mx-auto lg:grid lg:grid-cols-3 lg:gap-12">
        <div className="lg:col-span-2">
          <header className="mb-10">
            <h2 className="text-3xl font-black uppercase italic text-lime-400">Configurador de Liga</h2>
          ...
          {step === 1 && (
            <div className="space-y-6 animate-in slide-in-from-right duration-300">
              ...
              <div className="bg-zinc-900 p-6 rounded-3xl border border-zinc-800 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => setConfig({...config, hasPlayoffs: config.hasPlayoffs ? 0 : 1})} className={`w-12 h-6 rounded-full relative transition-colors ${config.hasPlayoffs ? 'bg-lime-400' : 'bg-zinc-800'}`}>
                    <div className={`absolute top-1 w-4 h-4 bg-zinc-950 rounded-full transition-all ${config.hasPlayoffs ? 'left-7 shadow-[-2px_0_5px_rgba(0,0,0,0.5)]' : 'left-1'}`} />
                  </button>
                </div>
                ...
                {config.hasPlayoffs === 1 && (
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-zinc-800 animate-in fade-in zoom-in">
                    <div>
                      ...
                      <select className="w-full bg-zinc-800 rounded-xl p-3 text-xs border border-zinc-700 outline-none focus:border-lime-400" value={config.playoffTeams} onChange={(e) => setConfig({...config, playoffTeams: parseInt(e.target.value)})}>
                    ...
                    <div>
                      ...
                      <select className="w-full bg-zinc-800 rounded-xl p-3 text-xs border border-zinc-700 outline-none focus:border-lime-400" value={config.playoffFormat} onChange={(e) => setConfig({...config, playoffFormat: e.target.value})}>
              ...
              <div className="bg-zinc-900 p-6 rounded-3xl border border-zinc-800">
                <label className="text-[10px] uppercase font-bold text-zinc-500 mb-2 block">Número de Equipos</label>
          ...
          {step === 3 && (
            <div className="space-y-6 animate-in slide-in-from-right duration-300">
              <div className="bg-zinc-900 p-6 rounded-3xl border border-zinc-800">
                ...
                <div className="flex gap-2">
                  <input id="venueSearch" type="text" placeholder="Nombre sede..." className="flex-1 bg-zinc-800 rounded-2xl py-4 px-5 text-sm outline-none border border-zinc-700 focus:border-lime-400" onKeyDown={(e) => e.key === 'Enter' && handleOpenModal()} />
                  <button onClick={handleOpenModal} className="bg-lime-400 text-zinc-950 px-6 rounded-2xl font-black text-xl hover:bg-lime-300 transition-colors">+</button>
                </div>
              </div>
              <div className="space-y-4">
                {config.selectedVenues.map((venue) => (
                  <div key={venue.id} className="bg-zinc-900/50 p-6 rounded-3xl border border-zinc-800 space-y-4 shadow-inner">
                    <div className="flex justify-between items-start">
                      <div><h3 className="font-bold text-lime-400 uppercase text-sm">📍 {venue.name}</h3><p className="text-[9px] text-zinc-500 font-bold uppercase">{venue.city}</p></div>
                      <button onClick={() => addSlotToVenue(venue.id)} className="text-[9px] bg-zinc-800 px-4 py-2 rounded-xl font-bold border border-zinc-700 hover:bg-zinc-700">+ Franja</button>
                    </div>
                    <div className="space-y-2">
                      {venue.slots.map((slot, idx) => (
                        <div key={idx} className="grid grid-cols-12 gap-2 pt-3 border-t border-zinc-800/50 items-center">
                          <input type="text" placeholder="Nombre campo" className="col-span-4 bg-zinc-900 rounded-xl p-2 text-[10px] font-bold outline-none border border-transparent focus:border-zinc-700" value={slot.fieldName} onChange={(e) => updateSlot(venue.id, idx, 'fieldName', e.target.value)} />
          ...
          {step === 4 && (
            <div className="animate-in zoom-in duration-300">
              ...
              {loading && <p className="text-center text-lime-400 font-bold mt-4 animate-pulse uppercase text-[10px] tracking-widest">Iniciando transacción en Railway...</p>}
        ...
        <div className="hidden lg:block">
          <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[2rem] sticky top-10 shadow-2xl">
            <h3 className="text-lime-400 font-black italic uppercase mb-6 text-sm tracking-widest text-center border-b border-zinc-800 pb-4">Resumen</h3>
            <div className="space-y-6">
              ...
              <div className="flex justify-between items-end border-b border-zinc-800 pb-2"><span className="text-[10px] text-zinc-500 uppercase font-bold">Ida/Vuelta</span><span className="text-sm font-bold">{config.hasReturnMatch ? 'SÍ' : 'NO'}</span></div>
            ...
            <div className="mt-8 p-4 bg-zinc-950 rounded-2xl border border-zinc-800">
              <p className="text-[8px] text-zinc-600 uppercase font-bold text-center leading-relaxed">Completa los 4 pasos para activar la liga en la base de datos oficial de FutNex.</p>
            </div>
          </div>
        </div>
      </div>
